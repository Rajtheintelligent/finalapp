...
# ------------------------------------------------------------------
# CORRECT IMPORT FOR MULTIPAGE NAVIGATION (NO SILENT FAILURE)
# ------------------------------------------------------------------
try:
    from streamlit_extras.switch_page_button import switch_page
except ImportError:
    st.error("⚠ Missing dependency: Please install with\n\npip install streamlit-extras")
    st.stop()
...
with col_left:
    with st.container(border=True):
        ...
        st.markdown(
            ...
            "Manage classes, batches, teachers, and students."
            "</div>",
            unsafe_allow_html=True
        )
        st.write("")
...
with col_center:
    ...
    with st.container(border=True):
        ...
        st.markdown(
            "<div style='font-size:14px; color:grey;'>Select a subject to continue.</div>",
            unsafe_allow_html=True
        )
        st.write("")
...
with col_right:
    with st.container(border=True):
        st.markdown("### **Payment**")
        st.markdown(
            "<div style='font-size:14px; color:grey;'>Billing, UPI payment and pricing.</div>",
            unsafe_allow_html=True
        )
        st.write("")
